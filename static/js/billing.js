...
/* Format amount - int if whole, else max 2 decimal */
function fmtAmt(n) {
  return Number.isInteger(n) || n % 1 === 0 ? Math.round(n).toString() : parseFloat(n.toFixed(2)).toString();
}
...
function calcTotal() {
  ...
  if (gtEl) gtEl.textContent = '₹' + fmtAmt(grand);
    ...
    if (!cart.length && bal <= 0) {
      ...
      var h = cart.map(function(i) {
        return '<div style="display:flex;justify-content:space-between;font-size:.84rem;padding:4px 0;border-bottom:1px solid #F1F5F9;">' +
          ...
          '<span style="font-weight:600;color:#0F172A;">₹' + fmtAmt(i.total) + '</span>' +
        '</div>';
      }).join('');
...
/* ── SHOW SUCCESS MODAL ── */
function showSucc(b) {
  document.getElementById('succBillNo').textContent = b.bill_number + ' · ₹' + fmtAmt(b.total_amount);
    ...
    '<div id="bill-render" style="font-family:Inter,system-ui,sans-serif;background:#FFFFFF;border:1px solid #E2E8F0;border-radius:16px;overflow:hidden;">'+
      ...
      '<div style="padding:18px 22px;">'+
        ...
        '<div style="display:flex;padding:8px 0;border-bottom:1px solid #E2E8F0;">'+
          '<span style="flex:2.5;font-size:.7rem;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.6px;">Item</span>'+
        ...
        '<div style="margin:20px 4px 4px;background:#1E2D4A;border-radius:18px;padding:22px 28px;display:flex;justify-content:space-between;align-items:center;box-shadow:0 6px 20px rgba(15,23,42,.25);">'+
          ...
          '<span style="color:#FBBF24;font-weight:800;font-size:1.4rem;font-family:Georgia,serif;white-space:nowrap;">₹'+fmtAmt(b.total_amount)+'</span>'+
        '</div>'+
