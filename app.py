...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'pottery-secret-2024')
...
class Product(db.Model):
    id           = db.Column(db.Integer, primary_key=True)
    telugu_name  = db.Column(db.String(120), nullable=False)
    english_name = db.Column(db.String(120), nullable=False)
...
def settings():
    ...
    return render_template('settings.html', current_lang=session.get('lang', 'en'))
...
def products():
    ...
    if search:
        q = q.filter(db.or_(
            Product.telugu_name.ilike(f'%{search}%'),
...
def create_bill():
    ...
    for item in items:
        ...
        if price <= 0:
            return jsonify({'error': f"Enter price for {item.get('name')}"}), 400
        enriched.append({
            'product_id':  item.get('product_id'),
            'name':        item.get('name'),
